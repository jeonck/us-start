---
title: 최고의 투자 — 일등주식투자법
---

# 🏆 최고의 투자 — 일등주식투자법

:::info 출처
이 페이지는 [최고의 투자 — 일등주식투자법](https://jeonck.github.io/adv-invest/#method) 의 내용을 요약한 것입니다.
상세 내용은 하단 링크를 참고하세요.
:::

## 한 줄 정의

> **"잃지 않는 것이 먼저."**
> 세계 시총 1등 주식만 보유하고, 나스닥 -3% 법칙으로 손실을 차단하는 원칙 기반 투자법. (조던 · 김장섭)

## 투자 의사결정 흐름

```mermaid
flowchart TD
    A(["🏆 세계 시총 1등 주식\n100% 보유"])
    B{"나스닥\n하루 -3% 이상?"}
    C{"2위가 10%\n이내 추격?"}
    D(["💵 즉시 현금화\n달러 대기"])
    E(["1등 50%\n2등 50% 분산"])
    F{"1등 교체?"}
    G(["🔄 새 1등으로\n전량 교체"])
    H{"30일간\n추가 -3% 없음?"}

    A --> B
    B -->|"아니오"| C
    B -->|"예"| D
    C -->|"아니오"| A
    C -->|"예"| E
    E --> F
    F -->|"아니오"| A
    F -->|"예"| G
    G --> A
    D --> H
    H -->|"아니오 — 대기 연장"| D
    H -->|"예 — 안정 확인"| A

    style A fill:#1a56db,color:#fff,stroke:#1a56db
    style D fill:#e3a008,color:#fff,stroke:#e3a008
    style G fill:#7e3af2,color:#fff,stroke:#7e3af2
    style E fill:#0e9f6e,color:#fff,stroke:#0e9f6e
```

## 5가지 핵심 원칙

| # | 원칙 | 내용 |
|---|------|------|
| 1 | **종목 선택** | 세계 시총 1등 주식만 100% 보유 |
| 2 | **매도 신호** | 나스닥 하루 -3% 이상 → 즉시 비중 축소 또는 전량 매도 |
| 3 | **재매수 조건** | 마지막 -3% 이후 30일간 추가 하락 없으면 재매수 |
| 4 | **단계적 헤지** | 전고점 대비 -2.5%마다 주식 10%씩 현금화 (말뚝 박기) |
| 5 | **1등 교체** | 시총 순위 역전 시 → 새 1등으로 전량 전환 |

## 투자 가능 종목 — 인덱스펀드 포함

개별 주식 외에 아래 인덱스펀드도 전략 종목으로 활용할 수 있다.

| 티커 | 이름 | 특징 |
|------|------|------|
| **QQQM** | Invesco Nasdaq-100 ETF | 나스닥 100 추종 · 장기 보유에 유리한 저비용 |
| **QLD** | ProShares Ultra QQQ | 나스닥 100 **2배 레버리지** · 변동성 2배이므로 -3% 시 최소 50% 즉시 현금화 |
| **SMH** | VanEck Semiconductor ETF | 반도체 섹터 집중 · 엔비디아 비중 최대 |

:::caution 레버리지 상품 주의
QLD는 변동성이 2배이므로 나스닥 -3% 신호 발생 시 일반 종목보다 더 신속하게 비중을 줄여야 한다.
:::

## 시총 1등 기업 변천

| 기간 | 1등 기업 |
|------|---------|
| 2012 ~ 2023 | 애플 (AAPL) |
| 2023 ~ 2024 | 마이크로소프트 (MSFT) |
| 2024 ~ 현재 | 엔비디아 (NVDA) — 약 $4.8조 |

## 장단점 요약

| 구분 | 내용 |
|------|------|
| ✅ 장점 | 감정 배제 · 명확한 기준 · 초보자 실행 가능 · 대형 하락장 자산 보호 |
| ⚠️ 주의 | 잦은 매매 시 세금·수수료 발생 · 신호 즉시 실행 규율 필요 |

---

## 상세 자료

| 섹션 | 링크 |
|------|------|
| 📌 전략 개요 | [바로가기](https://jeonck.github.io/adv-invest/#method) |
| 🏆 1등 주식 선택법 | [바로가기](https://jeonck.github.io/adv-invest/#method) |
| 📉 나스닥 -3% 법칙 | [바로가기](https://jeonck.github.io/adv-invest/#method) |
| 🔄 리밸런싱 전략 | [바로가기](https://jeonck.github.io/adv-invest/#method) |
| 🆕 진화된 전략 | [바로가기](https://jeonck.github.io/adv-invest/#method) |

:::warning 면책사항
이 내용은 교육 목적으로 작성되었으며 투자 권유가 아닙니다. 모든 투자는 본인의 판단과 책임 하에 이루어집니다.
:::
